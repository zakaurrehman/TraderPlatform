  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 24 }}>
        <div style={{ background: '#ffffff', border: '1px solid rgba(37,99,235,0.12)', borderRadius: 14, padding: 20 }}>
          ...
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[['Name', 'name', 'Exness', 'text'], ['Affiliate Link', 'link', 'https://...', 'url'], ['Min Deposit', 'minDeposit', '$10', 'text'], ['Regulation', 'regulation', 'FCA, CySEC', 'text'], ['Rating (1-5)', 'rating', '4.5', 'number']].map(([label, field, ph, type]) => (
              <div key={field}>
                <label style={{ color: '#55606f', fontSize: 12, display: 'block', marginBottom: 3 }}>{label}</label>
            ...
            <div>
              <label style={{ color: '#55606f', fontSize: 12, display: 'block', marginBottom: 3 }}>Description</label>
        ...
        <div>
          {brokers.map(b => (
            <div key={b.id} style={{ background: '#ffffff', border: '1px solid rgba(37,99,235,0.08)', borderRadius: 12, padding: 14, marginBottom: 10, opacity: b.isActive ? 1 : 0.5 }}>
              ...
              <div style={{ color: '#7a8494', fontSize: 12 }}>{b.description}</div>
              <div style={{ display: 'flex', gap: 10, marginTop: 6, fontSize: 11, color: '#9aa3b2' }}>
          ...
          {brokers.length === 0 && <p style={{ color: '#9aa3b2' }}>No brokers yet.</p>}
        </div>
      </div>
    </div>
  )
